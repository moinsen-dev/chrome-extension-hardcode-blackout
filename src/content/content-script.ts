...
.blackout-status-indicator {
  position: fixed;
  bottom: 80px;  /* Moved up from 20px to avoid LinkedIn message overlap */
  right: 20px;
  background: rgba(33, 33, 33, 0.95);
  color: white;
  padding: 12px 16px;
...
.blackout-settings-btn:hover {
  background: rgba(255, 255, 255, 0.1);
}

.blackout-settings-btn svg {
  width: 16px;
  height: 16px;
  fill: #999;
...
.blackout-settings-btn:hover svg {
  fill: white;
}
...
export class PostDetector {
  ...
  private createStatusIndicator() {
    ...
    this.statusIndicator.innerHTML = `
      ...
      <div class="blackout-settings-btn" title="Open settings">
    ...
    const settingsBtn = this.statusIndicator.querySelector('.blackout-settings-btn');
    if (settingsBtn) {
